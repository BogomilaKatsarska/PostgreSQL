...
END;
$$
LANGUAGE plpgsql; --> Procedural Language Postgre SQL
 
SELECT fn_full_name('Bogomila', 'Katsarska') AS "Full Name"
...
$$
    ...
    BEGIN
        ...
        INTO  city_id;      --> присвои заявката към променливата city_id
...
-- Proceedure add person

CREATE PERSON sp_add_person(first_name VARCHAR, last_name VARCHAR, city_name VARCHAR)
AS 
$$
BEGIN
...
CALL sp_add_person('Mila', 'Mihaylova', 'Sofia');
DROP PROCEDURE sp_add_persohn;

-- RAISE NOTICE

CREATE FUNCTION fn_test_function(first_name VARCHAR, last_name VARCHAR)
RETURNS INT AS
$$
    BEGIN
...
-- Create trigger to keep only 10 records in log table - WITH LOOP
